...
This program contains tools for completing ancient Greek words ending with
elision (marked with apostrophe), so they can be processed by a lemmatizer. The
current version will not help with parsing, as it simply aims to produce A valid
...
Endings are completed in the following steps:
    ...
    3. All remaining words are ended with epsilon (seems the most common)

Next steps for improvement:
I developed the lists of predicted completions by looking at the ellided forms
in Euripides' Orestes. The function "sorted_elisions" included in this file
will helpfully extract elided words and sort them according to their ending.
...
be done by searching a large corpus for instances of the elided string with 
the addition of [ειαο] (taking into account roughening of π, κ, and τ).

(The replacement pairs are currently stored as a dictionary rather than a list
of tuples because I considered providing multiple options.)
...
# It would be good to make the list more systematic, though, eventually.
...
# first in the list, so that they aren't preempted by the broader patterns (e.g. -μ : -μα). 
# Also, endings typically followed by epsilon are omitted, as that is the default.
...
def sorted_elisions (string):
    """This function is helpful for building a list of elision replacements
    by looking at elided words with similar endings.  The function takes a
    string of greek text as input, tokenizes, extracts elided words, and sorts
    alphabetically according to the ending.  Return is a list.
    """
